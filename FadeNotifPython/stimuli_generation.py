...
def generate_vigilance_stimuli_csv(csv_file_name, total_duration_seconds):
    all_stimuli = []

    # select stimuli based on distribution
    stimuli_round = 0
    total_rounds = (total_duration_seconds * 1000) // (STIMULI_DURATION_MS * STIMULI_PER_ROUND)
    ...
    stimuli_csv_data = {
        'stimuli_type': stimuli_types,
        'stimuli_id': stimuli_ids,
        'image_id': image_ids
    }

    pd.DataFrame(data=stimuli_csv_data).to_csv(csv_file_name, mode='w', index=False)
    print('Generated vigilance stimuli csv: ', csv_file_name)


# generate_vigilance_stimuli_csv('stimuli/tmp.csv')
